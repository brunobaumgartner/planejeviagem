/**
 * TRAVELPAYOUTS DESTINATIONS - Busca destinos dinâmicos da API
...
export async function fetchPopularDestinations(origin: string = 'SAO'): Promise<DestinationData[]> {
  console.log(`[TPDestinations] 🔍 Buscando destinos populares de ${origin}`);

  try {
    // Verificar cache primeiro
    const cached = await getCachedDestinations(origin);
    if (cached) {
      console.log(`[TPDestinations] ✅ Cache hit - ${cached.destinations.length} destinos`);
      return cached.destinations;
    }

    console.log(`[TPDestinations] 📡 Cache miss - buscando da API`);
    ...
    if (!data.success || !data.data) {
      console.warn('[TPDestinations] ⚠️ API retornou sem dados');
      return [];
    }
    ...
    const destinations: DestinationData[] = Object.entries(data.data)
      ...
      .filter(dest => dest.averagePrice !== null) // Filtrar destinos sem preço
      .sort((a, b) => b.popularity - a.popularity) // Ordenar por popularidade
      .slice(0, 50); // Top 50 destinos

    console.log(`[TPDestinations] ✅ ${destinations.length} destinos encontrados`);
    ...
    console.error('[TPDestinations] ❌ Erro ao buscar destinos:', error);
    
    // Tentar retornar cache expirado como fallback
    const expiredCache = await getExpiredCache(origin);
    if (expiredCache) {
      console.warn('[TPDestinations] ⚠️ Usando cache expirado como fallback');
...
/**
 * Busca destinos do cache se ainda válido
 */
...
async function cacheDestinations(origin: string, destinations: DestinationData[]): Promise<void> {
  try {
    ...
    console.log(`[TPDestinations] 💾 Cache salvo (expira em ${CACHE_DURATION_HOURS}h)`);
  } catch (error) {
...
export async function refreshDestinationsCache(origin: string = 'SAO'): Promise<void> {
  console.log(`[TPDestinations] 🔄 Forçando atualização do cache para ${origin}`);
...
/**
 * Busca informações de uma cidade específica
 */
export async function getCityInfo(code: string): Promise<any> {
  try {
    const url = `https://api.travelpayouts.com/data/en/cities.json?code=${code}&token=${TRAVELPAYOUTS_TOKEN}`;
    const response = await fetch(url);
    ...
    console.error(`[TPDestinations] Erro ao buscar cidade ${code}:`, error);
    return null;
  }
}

/**
 * Busca informações completas de uma cidade pelo código IATA
 */
export async function getCityInfoByCode(code: string): Promise<DestinationData | null> {
  console.log(`[TPDestinations] 🔍 Buscando informações para código ${code}`);
  
  try {
    // Buscar no arquivo completo de cidades
    const cities = await fetchAllCitiesData();
    
    if (!cities || cities.length === 0) {
      console.warn('[TPDestinations] ⚠️ Nenhuma cidade disponível no cache');
      return null;
    }
    
    // Buscar cidade pelo código
    const city = cities.find((c: any) => c.code === code);
    
    if (!city) {
      console.warn(`[TPDestinations] ⚠️ Cidade com código ${code} não encontrada`);
      return null;
    }
    
    // Converter para formato DestinationData
    const destData: DestinationData = {
      code: city.code,
      name: city.name || code,
      country: city.country_name || 'Desconhecido',
    ...
    console.log(`[TPDestinations] ✅ Cidade encontrada: ${destData.name} (${destData.countryCode})`);
    return destData;
    
  } catch (error) {
    console.error('[TPDestinations] ❌ Erro ao buscar informações da cidade:', error);
    return null;
  }
}

/**
 * Busca dados completos de múltiplas cidades de uma vez
...
export async function enrichDestinationsWithCityData(destinations: DestinationData[]): Promise<DestinationData[]> {
  console.log(`[TPDestinations] 🔍 Enriquecendo dados de ${destinations.length} cidades...`);
  ...
  try {
    ...
    if (!citiesData || citiesData.length === 0) {
      console.warn('[TPDestinations] ⚠️ Não conseguiu buscar dados de cidades');
      return destinations;
    }
    
    // Criar mapa código -> dados
    ...
    console.log(`[TPDestinations] ✅ Dados enriquecidos com sucesso`);
    return enriched;
    
  } catch (error) {
    console.error('[TPDestinations] ❌ Erro ao enriquecer dados:', error);
    return destinations;
  }
}
...
async function fetchAllCitiesData(): Promise<any[]> {
  ...
  try {
    ...
    if (cached) {
      ...
      if (now < expiresAt) {
        console.log('[TPDestinations] ✅ Cache de cidades encontrado');
        return data.cities;
      }
    }
    
    console.log('[TPDestinations] 📡 Baixando arquivo de cidades da API...');
    ...
    console.log(`[TPDestinations] ✅ ${cities.length} cidades cacheadas por 7 dias`);
    
    return cities;
    
  } catch (error) {
    console.error('[TPDestinations] ❌ Erro ao buscar cidades:', error);
    return [];
  }
}

/**
 * Busca código IATA pelo nome da cidade
 * ATUALIZADO: Agora busca no arquivo completo de cidades, não apenas destinos populares
 */
export async function findCityCode(cityName: string, origin: string = 'SAO'): Promise<string | null> {
  console.log(`[TPDestinations] 🔍 Buscando código IATA para "${cityName}"`);
  ...
  try {
    ...
    let found = cities.find((city: any) => 
      city.name && city.name.toLowerCase() === normalized
    );
    
    // Se não encontrou, tentar match parcial
    if (!found) {
      found = cities.find((city: any) => 
        city.name && city.name.toLowerCase().includes(normalized)
      );
    }
    
    // Se ainda não encontrou, tentar match pelo código
    if (!found) {
      found = cities.find((city: any) => 
        city.code && city.code.toLowerCase() === normalized
      );
    }
    
    if (found) {
      console.log(`[TPDestinations] ✅ Cidade encontrada: ${found.name} (${found.code})`);
      return found.code;
    }
    
    console.warn(`[TPDestinations] ⚠️ Cidade "${cityName}" não encontrada no banco de dados`);
    return null;
    
  } catch (error) {
    console.error('[TPDestinations] ❌ Erro ao buscar cidade:', error);
    return null;
  }
}

/**
 * Estima custos de acomodação e gastos diários baseado no país/região
 */
export function estimateDestinationCosts(destination: DestinationData): {
  ...
  // Países baratos (Brasil e similares)
